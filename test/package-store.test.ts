...
const files = (name: string, body = "Doğrulanmış yöntem.") => ({
...
for (const backend of [
  ...
  test(`package store ${backend}: complete publication, CAS, pinned revisions and scope`, async () => {
    ...
    const identity = new IdentityService(storage.db),
      ...
      project = await identity.createProject(owner, "Package contract");
    const store = new PackageStore(storage, root),
      name = `method-${crypto.randomUUID()}`;
    try {
      ...
      const found = await store.search(owner, {
        projectId: project.id,
        query: "TÜRKÇE",
      });
      expect(found.items.some((item) => item.skill_id === first.skill_id)).toBe(
        true,
      );
      const competing = await Promise.allSettled(
        ["Birinci", "İkinci"].map((body) =>
          store.publish(owner, {
            name,
            scope: "project",
            projectId: project.id,
      ...
      const old = await store.files(owner, first.skill_id, first.revision);
      expect(old.files["SKILL.md"]!.toString()).toContain(
        "Doğrulanmış yöntem.",
      );
      const current = await store.authorizedSkill(owner, first.skill_id);
      expect(current.active_revision).not.toBe(first.revision);
      await expect(
        storage.db
          .updateTable("skills")
          .set({ active_revision: "missing" })
      ...
      await expect(
        store.publish(owner, {
          ...
          files: files(name, "Denied"),
        }),
      ).rejects.toMatchObject({ code: "skill_protected" });
      const badIdentity = { userId: owner.userId, tenantId: "other" };
      await expect(
        store.files(badIdentity, first.skill_id, first.revision),
      ).rejects.toMatchObject({ code: "skill_unavailable" });
      await expect(
        store.search(owner, { projectId: "unknown", query: "" }),
      ).rejects.toMatchObject({ code: "project_unavailable" });
      await writeFile(join(old.path, "references/details.md"), "altered");
      await expect(
        store.files(owner, first.skill_id, first.revision),
...
test("package paths reject traversal, Windows aliases, symlinks, hardlinks and Unicode collisions", async () => {
  for (const path of [
    ...
    expect(() => validateInventory([path])).toThrow();
  ...
  try {
    ...
    await expect(
      secureRead(join(root, "package"), "hardlinked"),
    ).rejects.toThrow();
...
test("missing references and script packages without executable contracts are rejected", () => {
  expect(() =>
    validatePackage("test", { "SKILL.md": files("test")["SKILL.md"] }),
  ).toThrow();
  expect(() =>
    validatePackage("test", {
      ...files("test"),
      "scripts/unsafe.js": Buffer.from("console.log('not run')"),
    }),
  ).toThrow();
