...
async function fixture(
  ...
  const root = await mkdtemp(join(tmpdir(), "forge-script-")),
  ...
  const files = {
    ...
    [`scripts/main.${extension}`]: Buffer.from(source),
      ...
      JSON.stringify({
        ...
        entrypoints: {
          calculate: {
            ...
            timeoutMs,
            tests: [
              { name: "double", input: { x: 3 }, expected: { value: 6 } },
            ],
          },
        },
      }),
  ...
  return {
    root,
    path,
    manifest: validatePackage("script-test", files),
    executor: new DockerExecutor(root),
  };
}
for (const runtime of ["node", "python"] as const)
  test(`real Docker ${runtime}: JSON contract, artifact, validation and container cleanup`, async () => {
    const source =
      runtime === "node"
        ? "const fs=require('fs');const x=JSON.parse(fs.readFileSync(0,'utf8')).x;fs.writeFileSync('/output/result.txt',String(x*2));console.log(JSON.stringify({value:x*2}));"
    ...
    try {
      const result = await f.executor.execute(f.path, f.manifest, "calculate", {
        x: 3,
      });
      expect(result.result).toEqual({ value: 6 });
      expect(result.artifacts[0]?.path).toBe("result.txt");
      expect((await f.executor.validate(f.path, f.manifest)).passed).toBe(true);
      await expect(
        f.executor.execute(f.path, f.manifest, "calculate", { x: "bad" }),
      ).rejects.toMatchObject({ code: "invalid_script_input" });
    } finally {
...
test("real sandbox timeout kills the container and all descendants", async () => {
  ...
  try {
    await expect(
      f.executor.execute(f.path, f.manifest, "calculate", { x: 3 }),
    ).rejects.toMatchObject({ code: "script_timeout" });
    const list = await command(
      "docker",
      ["ps", "--filter", "name=forge-", "--format", "{{.Names}}"],
      { timeoutMs: 3000 },
    );
    expect(
      list.stdout.split("\n").filter((name) => /^forge-[0-9a-f-]+$/.test(name)),
    ).toEqual([]);
  } finally {
    await rm(f.root, { recursive: true, force: true });
  }
