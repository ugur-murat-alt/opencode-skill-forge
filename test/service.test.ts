...
async function config() {
  const root = await mkdtemp(join(tmpdir(), "forge-service-"));
  roots.push(root);
  return localConfig(root);
}
describe("independent service", () => {
  test("concurrent initialization converges on one owner credential and endpoint", async () => {
    ...
    const values = await Promise.all(
      Array.from({ length: 10 }, () => localConfig(first.dataDir)),
    );
    expect(new Set(values.map((value) => value.token)).size).toBe(1);
    expect(new Set(values.map((value) => value.url)).size).toBe(1);
    expect((await stat(join(first.dataDir, "owner-token"))).mode & 0o777).toBe(
      0o600,
    );
    expect(await readFile(join(first.dataDir, "owner-token"), "utf8")).toBe(
      first.token,
    );
  });
  test("public probes reveal only readiness while detailed health remains private", async () => {
    const cfg = await config(),
      app = await createHttpServer(cfg);
    const headers = { host: new URL(cfg.url).host };
    try {
      expect(
        (await app.inject({ url: "/health/live", headers })).json(),
      ).toEqual({ status: "live" });
      let ready = await app.inject({ url: "/health/ready", headers });
      for (let i = 0; i < 100 && ready.statusCode === 503; i++) {
        await new Promise((resolve) => setTimeout(resolve, 10));
        ready = await app.inject({ url: "/health/ready", headers });
      }
      expect(ready.statusCode).toBe(200);
      expect(ready.json()).toEqual({ status: "ready" });
      expect((await app.inject({ url: "/health", headers })).statusCode).toBe(
        401,
      );
  ...
  test("health and MCP require owner identity and exact origin/host", async () => {
    ...
    try {
      ...
      expect(
        (await app.inject({ url: "/health", headers: { host: headers.host } }))
          .statusCode,
      ).toBe(401);
      expect(
        (
    ...
    try {
      for (let i = 0; i < 2; i++) {
        ...
        await client.connect(
          new StreamableHTTPClientTransport(new URL(`${cfg.url}/mcp`), {
  ...
  test("provider configuration cannot use ambient credentials, paid fallback or arbitrary URLs", async () => {
    ...
    await expect(
      resolveProvider(profile, async () => "test-secret", {
        allowedOrigins: ["https://api.openai.com"],
        local: false,
      }),
    ).rejects.toMatchObject({ code: "paid_model_disabled" });
    await expect(
      resolveProvider({ ...profile, allowPaid: true }, async () => undefined, {
        allowedOrigins: ["https://api.openai.com"],
        local: false,
      }),
    ...
    await expect(
      resolveProvider(
        { ...profile, allowPaid: true, baseUrl: "http://169.254.169.254" },
        async () => "test",
        { allowedOrigins: [], local: false },
      ),
    ).rejects.toMatchObject({ code: "provider_endpoint_denied" });
    const local = await resolveProvider(
      ...
      async () => undefined,
      { local: true, allowedOrigins: ["http://127.0.0.1:11434"] },
    );
