...
test("private SPR candidate enforces inventory, read-before-change, exact patch and fenced publication", async () => {
  ...
  try {
    const identity = new IdentityService(storage.db),
      ...
      project = await identity.createProject(owner, "Staging test");
    ...
    const run = (await queue.claim("test-worker", 30000))!,
      staging = new EvolutionStaging(store, owner, run);
    const call = async (name: string, args: any) =>
      staging
        ...
        .execute("call", args);
    await expect(
      call("select", { name: "test-method", scope: "project" }),
    ).rejects.toMatchObject({ code: "inventory_required" });
    await call("inventory", { query: "test-method" });
    await call("select", { name: "test-method", scope: "project" });
    await call("patch", {
      path: "SKILL.md",
      old_text: "",
      new_text:
    ...
    ).rejects.toMatchObject({ code: "read_before_change_required" });
    ...
    await call("finalize", { decision: "create", reason: "Validated package" });
    expect(staging.closed).toBe(true);
    await expect(
      call("patch", { path: "x", old_text: "", new_text: "y" }),
    ).rejects.toMatchObject({ code: "run_closed" });
    expect(
      (await store.search(owner, { projectId: project.id })).items,
    ).toHaveLength(1);
