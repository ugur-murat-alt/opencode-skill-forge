...
Skill işleri en fazla üç deneme sınırındadır. Prompt Editor işleri tek denemelidir: süresi dolan veya deneme hakkı kalmayan prompt işi fallback olur, sessiz ikinci model harcaması yapılmaz. İstemciye özgün metnin korunması sözleşmesi geçerlidir. Sağlayıcı yapılandırılmamışsa normal prompt işinde `model_missing` gerekçeli fallback, skill işinde başarısız sonuç kaydedilir; bunlar model kalite başarısı değildir.

`bun test test/worker-process.test.ts` gerçek ayrı süreç, SIGKILL, süreli lease, production Node worker, fencing, idempotency, prompt fallback ve SIGTERM kapanışını kontrol eder. Önce `bun run build:plugin` çalıştırın; test dağıtım CLI'sını kullanır. Geçici veri ve yalnız teste ait süreçler temizlenir. PostgreSQL crash/soak ve canlı model kurtarma ayrı kabul ölçütleridir.
