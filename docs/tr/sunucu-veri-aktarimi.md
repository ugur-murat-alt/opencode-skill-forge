...
```json
{
  ...
  "checksum": "kesif-manifestindeki-dosya-dizisi-sha256-degeri",
  "content_base64": "klasik-zip-bytelarinin-base64-degeri",
  "scope": "personal",
  "flags": { "managed": true, "protected": false, "pinned": false }
}
```

Desteklenen tür: yalnız `package` (scope: personal veya project; flags: managed/protected/pinned). Gönderilen içerik tek klasik skill kökü içeren ZIP, en fazla 5 MiB'dir.
...
- `POST /api/migrations/package/{receipt_id}/rollback` aynı kullanıcının aktarımını kendi değişim/başka referans korumalarıyla geri alır.
...
`migration-upload` mevcut read-only keşif manifestini ve seçim eşlemesini kullanır. Uzak eşlemede owner değeri `authenticated-user` olmalıdır; hedef kullanıcı bearer token'ın kimliğidir. Paket seçim alanları yerel aktarım belgesiyle aynıdır.
